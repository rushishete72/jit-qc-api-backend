...
/** @type {{db: pgp.IDatabase, pgp: pgp.IMain, dropAllTables: function}} */
...
 * @async
 * @param {string} dirPath - SQL फ़ोल्डर का पाथ
...
                    .sort(); // Sorts files numerically (e.g., 01, 02)
...
        // यह सिंटैक्स त्रुटि को ठीक करने की संभावना 99% है।
        sql = sql.replace(/^\uFEFF/u, '').replace(/\u00A0/g, ' ').trim(); 

        // 🔑 FIX: फ़ाइल सामग्री को BEGIN/COMMIT ट्रांजैक्शन ब्लॉक में लपेटें
        // यह सुनिश्चित करता है कि एक फ़ाइल विफल होने पर, कोई आंशिक अपडेट न हो।
...
    try {
        // 1. डेटाबेस साफ़ करें (DROP ALL TABLES)
        await dropAllTables(); 
